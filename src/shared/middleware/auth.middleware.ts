...
export class AuthMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      if (!token["a-t"] && !token["r-t"]) {
        return this.authService.UnauthorizedException(res, '토큰이 없습니다.');
      }
      ...
      if (!refreshPayload?.userId) {
        return this.authService.UnauthorizedException(res, '유효하지 않은 토큰입니다.');
      }
      ...
      return this.authService.UnauthorizedException(res, 'auth middleware 알수없는 에러', err);
    }
  }
}
